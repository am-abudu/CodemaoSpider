...
def main(wid,_path):
    try:
            global tamp
            tamp=True
            root_insert('本应用作者啊不嘟，星辰工作室版权所有')
            编辑框4.see(tk.END)
...
def ui(启动窗口):
        ...
        size = '%dx%d+%d+%d' % (450, 292, (screenwidth - 450) / 2, (screenheight - 292) / 2)
        启动窗口.geometry(size)
        ...
        编辑框1 = tk.Text(启动窗口,wrap=tk.NONE,font=ft)
        编辑框1.insert(tk.END,'')
        编辑框1.place(x=98,y=27,width=300,height=23)
        
        global 编辑框2
        编辑框2 = tk.Text(启动窗口,wrap=tk.NONE,font=ft)
        编辑框2.insert(tk.END,'')
        编辑框2.place(x=98,y=59,width=250,height=23)
        
        按钮2_标题 = tk.StringVar()
        按钮2_标题.set('浏览')
        按钮2 = tk.Button(启动窗口,textvariable=按钮2_标题,command=get_path)
        按钮2.place(x=354,y=57,width=44,height=26)
        ...
        global 编辑框4
        编辑框4 = tk.Text(启动窗口,wrap=tk.NONE,font=ft)
        编辑框4.insert(tk.END,'')
        编辑框4.place(x=98,y=89,width=300,height=164)
root = tk.Tk()            
